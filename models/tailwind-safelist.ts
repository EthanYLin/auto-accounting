/**
 * Tailwind CSS 动态类名 Safelist
 * 
 * 这个文件的目的是让 Tailwind 的扫描器能够找到所有从数据库动态加载的颜色类名。
 * 虽然这些类名不会在运行时被使用，但它们的存在确保了 Tailwind 会生成对应的 CSS。
 * 
 * ⚠️ 不要删除这个文件，即使它看起来没有被导入使用！
 */
...
// 文字颜色 - 亮色模式
export const TEXT_COLORS_LIGHT = [
...
// 组合所有颜色（用于 safelist）
export const ALL_DYNAMIC_COLORS = [
  ...BG_COLORS_LIGHT,
  ...TEXT_COLORS_LIGHT,
  ...BG_COLORS_DARK,
  ...TEXT_COLORS_DARK,
