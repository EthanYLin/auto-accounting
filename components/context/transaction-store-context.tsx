...
interface TransactionStoreContextValue {
  ...
  // ==================== 脏状态跟踪 ====================
  setTransactionDraft: (id: number, updater: (draft: TransactionContentDraft) => TransactionContentDraft) => void;
...
export function TransactionStoreProvider({ children }: { children: React.ReactNode }) {
  ...
  const query = useQuery({
    ...
    queryFn: async () => {
      const [txRes, splitsRes] = await Promise.all([getAllTransactions(), getAllTransactionSplits()]);
  ...
  const getBaseline = useCallback((id: number): TransactionWithRelations | undefined => {
    return (queryClient.getQueryData<TransactionWithRelations[]>(transactionQueryKey) ?? []).find(tx => tx.id === id);
  }, [queryClient, transactionQueryKey]);
  ...
  const setTransactionDraft = useCallback((id: number, updater: (draft: TransactionContentDraft) => TransactionContentDraft) => {
    setLocalEdits(prev => {
  ...
  const persistSingleTx = useCallback(async (id: number, tx: TransactionWithRelations): Promise<SaveResult> => {
  ...
  const clearMatchingLocalEdits = useCallback((ids: number[], draftSnapshot: Map<number, TransactionContentDraft>) => {
    setLocalEdits(prev => {
  ...
  const saveToServer = useCallback(async (id: number, draftOverride?: TransactionContentDraft): Promise<SaveResult> => {
    ...
    return offerSingleSave(async () => {
      try {
        ...
        queryClient.setQueryData<TransactionWithRelations[]>(transactionQueryKey, old => produce(old ?? [], draft => {
  ...
  }, [clearMatchingLocalEdits, getBaseline, localEdits, offerSingleSave, persistSingleTx, queryClient, saveState, transactionQueryKey]);

  const saveChildrenSelection = useCallback(async (parentId: number, selectedIds: number[]): Promise<SaveResult> => {
    ...
    for (const childId of Array.from(addedSet)) {
      ...
      if (!child) continue;
      if (child.parent_id !== null && child.parent_id !== parentId) oldParentSet.add(child.parent_id);
      child.children_ids.forEach(gcId => orphanedChildSet.add(gcId));
    }
    const affectedIds = new Set([parentId, ...Array.from(removedSet), ...Array.from(addedSet), ...Array.from(oldParentSet), ...Array.from(orphanedChildSet)]);
    ...
    return offerExclusiveAction(async () => {
      ...
      queryClient.setQueryData<TransactionWithRelations[]>(transactionQueryKey, old => produce(old ?? [], draft => {
        ...
        if (parent) { parent.status = '待处理'; parent.children_ids = [...selectedIds]; }
        removedSet.forEach(id => {
          const tx = draftMap.get(id);
          if (tx) { tx.parent_id = null; tx.status = '待处理'; }
        });
        oldParentSet.forEach(id => {
          const tx = draftMap.get(id);
          if (tx) { tx.status = '待处理'; tx.children_ids = tx.children_ids.filter(cid => !addedSet.has(cid)); }
        });
        ...
        addedSet.forEach(id => {
          const tx = draftMap.get(id);
          if (tx) { tx.parent_id = parentId; tx.status = '附加到其他交易'; tx.splits = []; tx.children_ids = []; }
        });
      }));
      ...
      setLocalEdits(prev => {
        ...
        next.forEach((draft, draftId) => {
          if (draftId === parentId || removedSet.has(draftId) || oldParentSet.has(draftId) || orphanedChildSet.has(draftId)) {
      ...
      try {
        ...
        oldParentSet.forEach(id => txUpdates.set(id, { ...txUpdates.get(id), status: '待处理' }));
        removedSet.forEach(id => txUpdates.set(id, { ...txUpdates.get(id), parent_id: null, status: '待处理' }));
        orphanedChildSet.forEach(id => txUpdates.set(id, { ...txUpdates.get(id), parent_id: null, status: '待处理' }));
        addedSet.forEach(id => txUpdates.set(id, { ...txUpdates.get(id), parent_id: parentId, status: '附加到其他交易' }));
        ...
        await queryClient.invalidateQueries({ queryKey: transactionQueryKey });
        return { success: false, error: error instanceof Error ? error.message : '保存父子关系失败' };
      }
    });
  ...
  const createEmptyTransaction = useCallback(async () => {
    ...
    try {
      ...
      const result = await insertTransaction({
        account_id: accounts[0].id, amount: 0, datetime: now, source: '手动新建', status: '待处理',
      });
      if (!result.success || !result.data) return { success: false, error: result.error || '新建记录失败' } as const;
      ...
      queryClient.setQueryData<TransactionWithRelations[]>(transactionQueryKey, old =>
        [createdTx, ...(old ?? []).filter(tx => tx.id !== createdTx.id)]
      );
  ...
  const deleteTransactions = useCallback(async (ids: number[]) => {
    ...
    const orphanedChildIds = Array.from(new Set(ids.flatMap(id => getBaseline(id)?.children_ids ?? []))).filter(id => !idsToDelete.has(id));
    ...
    try {
      ...
      for (const childId of orphanedChildIds) {
        ...
        if (!r.success) throw new Error(r.error || `父交易被删除，更新子交易 #${childId} 关系失败`);
      }
      ...
      await queryClient.invalidateQueries({ queryKey: transactionQueryKey });
      return { success: false, error: error instanceof Error ? error.message : '删除交易记录失败' } as const;
    }
  ...
  const value: TransactionStoreContextValue = useMemo(() => ({
    ...
    error: query.error ? (query.error instanceof Error ? query.error.message : '加载交易数据失败') : null,
    ...
    transactions, query.isLoading, query.fetchStatus, query.error, query.isSuccess, saveState, isDirty, getDirtyIds,
    loadTransactions, setTransactionDraft, saveToServer,
    saveChildrenSelection, discardChanges, discardAllChanges, createEmptyTransaction, deleteTransactions,
  ]);
