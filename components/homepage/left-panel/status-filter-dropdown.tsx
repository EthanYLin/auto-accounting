        ...
        <>
          ...
          {ALL_TRANSACTION_STATUS.map((status) => {
            return (
              <DropdownItem 
                key={status.name}
                startContent={statusFilter === status.name ? <CheckIcon className="w-4 h-4" /> : <span className="w-4" />}
              >
                <div className="flex items-center justify-between w-full gap-4">
                  <span>{status.name}</span>
                  <Chip size="sm" color={status.chipColor} variant="flat">{stats[status.name] || 0}</Chip>
                </div>
              </DropdownItem>
            );
          })}
