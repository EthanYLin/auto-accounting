...
type AppDataListItem<K extends AppDataListKey> = AppDataValue[K][number] &
  EntityWithId;
...
> = {
  ...
  update: (
    id: number,
    payload: TPayload,
  ) => Promise<ActionResult<AppDataListItem<K>>>;
...
function unwrapMutationResult<T>(
...
function removeAppDataItem<K extends AppDataListKey>(
  items: AppDataValue[K],
  id: number,
) {
...
function invalidateAppDataInBackground(queryClient: QueryClient) {
  void queryClient
    .invalidateQueries({ queryKey: appDataQueryKey })
    .catch(() => undefined);
}
...
>({
  ...
  const createMutation = useMutation({
    ...
    onSuccess: (entity) => {
      updateAppDataList(queryClient, listKey, (items) =>
        upsertAppDataItem(items, entity),
      );
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: (_data, variables) => {
      updateAppDataList(queryClient, listKey, (items) =>
        removeAppDataItem(items, variables.id),
      );
      addToast({
...
export function useBudgetMutations() {
  const { save, deleteItem, isSaving, isDeleting } = useSettingsEntityMutations(
    {
      listKey: "budgetTypes",
      create: ({ name, icon }: { name: string; icon?: string }) =>
        createBudgetType(name, icon),
  ...
  const { save, deleteItem, isSaving, isDeleting } = useSettingsEntityMutations(
    {
      ...
      messages: {
        ...
        deleteSuccess: (variables: { id: number }) =>
          `匹配规则 #${variables.id} 已删除`,
